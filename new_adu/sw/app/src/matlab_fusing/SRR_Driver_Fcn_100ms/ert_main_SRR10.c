...
#include "SRR_Driver_Fcn_100ms.h"      /* Model's header file */
#include "rtwtypes.h"
...
void rt_OneStep(void)
{
  ...
  /* Check for overrun */
  if (OverrunFlag) {
    rtmSetErrorStatus(SRR_Driver_Fcn_100ms_M, "Overrun");
    return;
  }
...
int_T main(int_T argc, const char *argv[])
{
  ...
  fflush((NULL));
  while (rtmGetErrorStatus(SRR_Driver_Fcn_100ms_M) == (NULL)) {
  ...
  /* Terminate model */
  SRR_Driver_Fcn_100ms_terminate();
  return 0;
}
